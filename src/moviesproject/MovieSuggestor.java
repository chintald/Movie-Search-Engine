...
public class MovieSuggestor {
	...
	public static void showSuggestionsBasedOnMovie(GraphProcessingUtils customGraph, JSONObject movieForSuggestion) {
		...
		if (listBasedOnGenre.isEmpty() && listBasedOnActors.isEmpty()) {
			...
			suggestedMovies = new HashSet<>(listBasedOnGenre);
			suggestedMovies.retainAll(listBasedOnActors);
			if (suggestedMovies.size() <= 3) {
				System.out.println(suggestedMovies.addAll(listBasedOnGenre));
			}
		}
		...
		for (int i = 0; i < (movieArr.length >= 6 ? 6 : movieArr.length); i++) {
			if (GraphProcessingUtils.getCodeOfMovie(movieForSuggestion.get("movieName")
					.toString()) == GraphProcessingUtils.getCodeOfMovie(movieArr[i].getMovieName())) {
				continue;
			}
			System.out.println("=> " + GraphProcessingUtils.getCodeOfMovie(movieArr[i].getMovieName()) + ": "
					+ movieArr[i].getMovieName());
		}
		System.out.println("=================================");
